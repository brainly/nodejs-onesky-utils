 ...
 * @param  {Number} options.importId Import ID
...
function getImportTask(options) {
  ...
  return _private.makeRequest(_getLink(options), 'Unable to fetch import task info');
}
...
function _getLink(options) {
  ...
  return `${apiAddress}/1/projects/${options.projectId}/import-tasks/${options.importId}?${query}`;
}
