...
chai.use(sinonChai);
describe('GET import task info with wrong credentials', function () {
  before(function () {
  ...
  it('Return error when miss importId', function () {
    requestPromise = new Promise(function (resolve, reject) {
      reject({
        response: {
          body: '{"meta":{"status":400,"message":"Invalid import ID"},"data":{}}'
        }
      });
    });

    oneskyUtils.getImportTask(defaultOptions)
      .then(function (data) {
        expect(data).to.be.undefined;
      })
      .catch(function (error) {
        expect(error.code).to.equal(400);
        expect(error.message).to.equal('Invalid import ID');
      });
  });
